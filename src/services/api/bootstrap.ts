...
import {
  ...
  getOpenAICompatModelsEndpoint,
  OpenAICompatAuthError,
} from './openaiCompatModels.js'
...
async function fetchOpenAICompatBootstrapData(): Promise<BootstrapResponse | null> {
  const endpoint = getOpenAICompatModelsEndpoint(process.env.OPENAI_BASE_URL)
  logForDebugging(`[Bootstrap] Fetching OpenAI-compatible models from ${endpoint}`)

  try {
    const additionalModelOptions = await fetchOpenAICompatModelOptions({
      apiKey:
    ...
    logForDebugging('[Bootstrap] OpenAI-compatible models fetched')
    return {
      client_data: null,
    ...
    if (error instanceof OpenAICompatAuthError) {
      logForDebugging('[Bootstrap] Skipped: missing OpenAI-compatible auth')
      return null
    }
...
async function fetchBootstrapAPI(): Promise<BootstrapResponse | null> {
  ...
  if (apiProvider === 'openaiCompat') {
    return fetchOpenAICompatBootstrapData()
  }
